...
def get_filtered_events(
    ...
    # Product floor: incident feed only shows events with a named victim.
    # Actor-only events (campaign/advisory reports with no named org) are
    # threat intelligence, not incident reports — they don't belong here.
    if signal_type == "incident":
        query = query.filter(CyberEvent.victim_org_name.isnot(None))
